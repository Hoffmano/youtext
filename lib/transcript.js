(function (root, factory) {
  ...
  function transcriptButton(root) {
    const scoped = root.querySelector('ytd-video-description-transcript-section-renderer button');
    if (scoped) return scoped;
    const buttons = [...root.querySelectorAll('button'), ...root.querySelectorAll('[role="button"]')];
    return buttons.find((button) => {
      const label = `${button.getAttribute('aria-label') || ''} ${button.textContent || ''}`;
      return !button.closest('ytd-engagement-panel-section-list-renderer') && /transcri|transcript/i.test(label);
    }) || null;
  }

  function segmentText(segment) {
    const textNode = segment.querySelector('.ytAttributedStringHost, span[role="text"], .yt-core-attributed-string, .segment-text, yt-formatted-string');
  ...
  function transcriptSegments(root) {
    return root.querySelectorAll('transcript-segment-view-model, ytd-transcript-segment-renderer, ytd-transcript-segment-list-renderer [data-start-ms]');
  }
